...
def main():
    ...
    if args.ckpt:
        ...
        torch.save({"net": ck["net"], "args": dict(ck["args"], algo="alphazero", share="embeddings"),
                    "value_norm": ck.get("value_norm")}, snap)
    else:
        net = FactoryNet(a.d_emb, a.d, a.layers, a.heads, share="embeddings")
        torch.save({"net": net.state_dict(), "args": dict(vars(a), algo="alphazero", share="embeddings"),
    ...
    pool = AZ.WorkerPool(args.workers, AZ.worker_cfg(a), server, seed=1)
    try:
    ...
    n = tot["b_decisions"]
    print(f"workers {args.workers}  sims {a.sims}  dets {a.dets}  t_budget {a.t_budget}")
